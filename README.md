...
2. 进入建立好的==模拟系统==后就可以创建==规则==，规则由一组http request(==请求==)和http response(==响应==)组成，请求和响应的具体编写方式可以查看规则编辑页面的[帮助文档](http://mock-api.com/rule-help.html)。规则创建时可以指定属于哪一个api以及tag(分类标签)，后续可以通过api和tag来查找和过滤规则。
...
![](http://cms.agilean.cn/WechatIMG1316.jpeg)
